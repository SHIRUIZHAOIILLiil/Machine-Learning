...
tokenList = tokenizer.texts_to_sequences([seed_text])[0]
tokenList = tf.keras.preprocessing.sequence.pad_sequences([tokenList], maxlen=max_sequence_len - 1, padding='pre')

# 返回的是每个类别的概率
print(model.predict(tokenList))
# argmax 找到最大值的索引，axis=-1 返回最后一个轴上最大值的索引，0表示找每一列的最大值，1表示找每一行的最大值；0维和1维
# np创建数组，可以是（2，3，4）这个形状，表示两个3x4的矩阵叠在一起，0表示2，3表示行，4表示列，对应axis0，1，2
predictions = np.argmax(model.predict(tokenList), axis=-1)
print(predictions)
